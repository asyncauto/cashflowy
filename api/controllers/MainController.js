...
module.exports = {
	...
	listRules:function(req,res){
		async.auto({
			getRules:function(callback){
				Rule.find({user:req.user.id}).exec(callback);
			}
		},function(err,results){
			var locals={}
	...
	createRule:function(req,res){
		// this just creates a black rule and takes the user to edit rule page.
		var rule = {
			user:req.user.id,
			type:'user',
		}
		Rule.create(rule).exec(function(err,rule){
			res.redirect('/rule/'+rule.id+'/edit');
		})
	},
	...
	viewPnL:function(req,res){
		...
		async.auto({
			...
			var categories = GeneralService.orderCategories(results.getAllCategories);
			var head_cat=_.find(categories,{id:results.getPnl.details.pnl_head_category});
			// var head = Math.floor(Math.random() * categories.length);
			locals.pnl.statement={
				head:[
					{
